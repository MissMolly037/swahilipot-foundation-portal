...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for dept in dept_objs.values():
            DepartmentChannel.objects.get_or_create(
                department=dept, name=f"{dept.name} Updates"
            )
        ...
        if admin:
            ...
            self.stdout.write(
                self.style.WARNING(
                    "No superuser found. Run createsuperuser, then run seed_data again to mark that account as Admin."
                )
            )
        Announcement.objects.get_or_create(
            title="Welcome to Swahilipot Portal",
            defaults={
                "content": "Use this portal for attendance, tasks, events, communication, and feedback.",
                "created_by": admin,
            },
        )
        Event.objects.get_or_create(
            ...
            defaults={
                "description": "Monthly forum for program updates and community learning.",
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Seed data created. The superuser account is used as the Admin account."
            )
        )

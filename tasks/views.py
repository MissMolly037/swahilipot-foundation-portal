...
def task_list(request):
    ...
    return render(
        request, "tasks/list.html", {"tasks": tasks, "today": timezone.localdate()}
    )
...
def task_update(request, pk):
    ...
    if "status" in request.POST:
        if not _can_update_status(request.user, task):
            messages.error(
                request, "You do not have permission to update this task's status."
            )
        ...
        if form.is_valid():
            ...
            for recipient in {task.assigned_to, task.assigned_by} - {
                None,
                request.user,
            }:
                notify_user(

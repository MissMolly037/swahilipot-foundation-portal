...
def event_edit(request, pk):
    ...
    return render(
        request, "form.html", {"form": form, "title": f"Edit Event: {event.title}"}
    )
...
def event_detail(request, pk):
    ...
    checked_in = EventCheckIn.objects.filter(
        event=event, participant=request.user
    ).exists()
...
def event_checkin(request, pk):
    ...
    if not EventRegistration.objects.filter(
        ...
        return JsonResponse(
            {"ok": False, "error": "You are not registered for this event."}, status=403
        )

    if EventCheckIn.objects.filter(event=event, participant=request.user).exists():
        return JsonResponse(
            {"ok": False, "already": True, "message": "You have already checked in."}
        )
    ...
    try:
        ...
        return JsonResponse(
            {"ok": False, "error": "Invalid or missing location data."}, status=400
        )
...
def no_shows(request, pk):
    ...
    checked_in_ids = set(
        EventCheckIn.objects.filter(event=event).values_list(
...
def no_shows_report(request, pk, fmt):
    ...
    return pdf_response(
        filename, f"No-Shows: {event.title}", headers, rows, subtitle=subtitle
    )
...
def not_registered(request, pk):
    ...
    users = (
        ...
        .exclude(pk__in=registered_ids)
        .order_by("first_name", "username")
    )
    return render(
        request,
        "events/not_registered.html",
        {
            "event": event,
            "users": users,
            "total": users.count(),
...
def not_registered_report(request, pk, fmt):
    ...
    if fmt == "xlsx":
        return excel_response(filename, headers, rows, subtitle=subtitle)
    return pdf_response(
        filename, f"Not Registered: {event.title}", headers, rows, subtitle=subtitle
    )
...
def report(request, pk, fmt):
    ...
    form_rows = list(
        event.form_responses.order_by("submitted_at").values_list(
            "respondent_name",
            "respondent_email",
            "respondent_phone",
            "submitted_at",
    ...
    portal_rows = [
        ...
        for r in event.registrations.select_related("participant").order_by(
            "registration_date"
        )
    ]
    ...
    return pdf_response(
        filename, f"Registrations: {event.title}", headers, numbered, subtitle=subtitle
    )
...
def form_response_webhook(request, pk):
    ...
    nested = (
        body.get("responses", {}) if isinstance(body.get("responses"), dict) else {}
    )
    name = _extract_field(
        body, ["name", "full_name", "Full Name", "respondent_name"]
    ) or _extract_field(nested, ["name", "full_name", "Full Name"])
    email = _extract_field(
        body, ["email", "Email Address", "Email", "respondent_email"]
    ) or _extract_field(nested, ["email", "Email Address", "Email"])
    phone = _extract_field(
        body, ["phone", "Phone Number", "Phone", "respondent_phone"]
    ) or _extract_field(nested, ["phone", "Phone Number", "Phone"])
    ...
    anon = (
        FormResponse.objects.filter(
            ...
            respondent_name="",
            respondent_email="",
            raw_data__source="portal_registration",
            submitted_at__gte=two_hours_ago,
        )
        .order_by("-submitted_at")
        .first()
    )
    if anon:
        anon.respondent_name = name
        anon.respondent_email = email
        anon.respondent_phone = phone
        anon.raw_data = body
        anon.save(
            update_fields=[
                "respondent_name",
...
def form_response_webhook_byid(request):
    ...
    if event is None:
        try:
            ...
            event = (
                Event.objects.filter(title__iexact=eid, end_date__gte=timezone.now())
                .order_by("start_date")
                .first()
            )
            if not event:
                return JsonResponse(
                    {"error": f"No active event named '{eid}'"}, status=404
                )
    if event.form_response_count >= event.capacity:
        return JsonResponse(
            {

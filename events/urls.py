...
urlpatterns = [
    ...
    # Live count polling
    path(
        "<int:pk>/registration-count/",
        views.registration_count_api,
    ...
    path(
        "<int:pk>/no-shows/report/<str:fmt>/",
        views.no_shows_report,
        name="no_shows_report",
    ),
    # Not registered — portal users who never signed up
    path("<int:pk>/not-registered/", views.not_registered, name="not_registered"),
    path(
        "<int:pk>/not-registered/report/<str:fmt>/",
        views.not_registered_report,
        name="not_registered_report",
    ),
    # Apps Script webhook (optional enrichment)
    path(
        "<int:pk>/form-response/",
        views.form_response_webhook,
        name="form_response_webhook",
    ),
    path(
        "form-response/",
        views.form_response_webhook_byid,
        name="form_response_webhook_byid",
    ),
]

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        guard_title = (
            f"📊 Weekly attendance report — w/e {week_end.strftime('%d %b %Y')}"
        )
        ...
        if not options.get("force"):
            ...
            if already:
                self.stdout.write(
                    f"  Report for w/e {week_end} already sent today — skipping."
                )
                return
        ...
        if not managers:
            self.stdout.write(
                self.style.WARNING("  No managers with email addresses — skipping.")
            )
            return
        ...
        for user in staff:
            ...
            if days_absent > 0 or lates > 0 or early_outs > 0:
                flags = []
                if days_absent:
                    flags.append(f"{days_absent} absent")
                if lates:
                    flags.append(f"{lates} late arrival{'s' if lates!=1 else ''}")
                if early_outs:
                    flags.append(
                        f"{early_outs} early departure{'s' if early_outs!=1 else ''}"
                    )
        ...
        for mgr in managers:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f"  ✗ Failed for {mgr.email}: {exc}")
                )
        ...
        notify_managers(
            ...
            message=(
                ...
                + (
                    f" {len(summary_lines)} staff member(s) had attendance issues."
                    if summary_lines
                    else ""
                )
            ),
    ...
    def _build_excel(self, headers, rows, week_start, week_end, working_days):
        ...
        def font(color="000000", bold=False, size=11, italic=False):
            return Font(
                color=color, bold=bold, size=size, italic=italic, name="Calibri"
            )
        ...
        ws["A2"] = (
            f"  Weekly Attendance Report  |  {week_range_str}  |  {working_days} working days"
        )
        ...
        total_hours = sum(
            float(r[6].replace("h", "")) for r in rows if isinstance(r[6], str)
        )

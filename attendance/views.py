...
def check_out(request):
    ...
    if dep == Attendance.DepartureStatus.LEFT_EARLY:
        ...
        notify_managers(
            title=f"🚪 Early check-out — {_name}",
            message=(
                f"{_name} checked out at {_actual_str}, which is {_early_str} before "
                f"the expected end time of {_expected_str} at {site.name}. "
                f"Total hours recorded: {record.total_hours}h."
            ),
